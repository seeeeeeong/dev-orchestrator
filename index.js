...
async function cleanupBranch(dir, branch, baseBranch) {
  ...
  try {
    await runCmd(`git push origin --delete ${branch}`, dir);
  } catch {}
}
...
async function squashBranchCommits(dir, branch, baseBranch) {
  ...
  const commitMsg = await generateCommitMsg(dir, '작업 커밋 squash');
...
async function doWork(projectName, prompt, message) {
  ...
  if (!reviewPassed) {
    ...
    await message.channel.send('🗑️ 브랜치 삭제 완료. `!work`로 다시 시도하세요.');
